...
import { X, Ticket, AlertTriangle, ShoppingCart, CheckCircle, Clock, TrendingUp, Zap, Shield, Sparkles, Plus, Minus, Radio } from "lucide-react"
...
const BuyTicketDialog: React.FC<BuyTicketDialogProps> = ({
  ...
  const isTicketTypeSoldOut = (ticket: TicketType) => {
    ...
    return (ticket.ticketSale || 0) >= ticket.availableTickets
  }

  const getRemainingTickets = (ticket: TicketType) => {
    if (!ticket.availableTickets) return null
    return Math.max(0, ticket.availableTickets - (ticket.ticketSale || 0))
  }

  const getPercentageSold = (ticket: TicketType) => {
    if (!ticket.availableTickets) return 0
    return Math.round(((ticket.ticketSale || 0) / ticket.availableTickets) * 100)
  }
  ...
  const updateQuantity = (ticketPolicy: string, newQuantity: number) => {
    ...
    if (hasLimit) {
      const availableCount = Math.max(0, (ticketData?.availableTickets || 0) - (ticketData?.ticketSale || 0))
  ...
  return (
    <>
      {/* Inline styles for animations that can't be done with Tailwind alone */}
      <style>{`
        ...
        .today-banner-border {
          background: linear-gradient(90deg, #c026d3, #7c3aed, #2563eb, #7c3aed, #c026d3);
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl w-full max-w-3xl my-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col max-h-[90vh] overflow-hidden">

          {/* ── Dialog header ──────────────────────────────────────────── */}
          <div className="bg-gradient-to-r from-[#6b2fa5] via-purple-600 to-[#6b2fa5] px-6 md:px-8 py-5 flex items-center justify-between flex-shrink-0">
          ...
          {isEventToday && !isEventPassed && (
            <div className="flex-shrink-0">
              ...
              <div className="today-banner-border h-[2px] w-full" />

              <div className="today-banner px-5 py-3.5 flex items-center justify-between gap-4 overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden min-w-0 relative">
                  ...
                  <div className="flex whitespace-nowrap ticker-track">
                    ...
                    {[0, 1].map((i) => (
                      <span key={i} className="inline-flex items-center gap-6 pr-16 text-[11px] font-semibold tracking-widest text-white/50 uppercase">
                        <span className="text-white/90 font-black tracking-[0.15em]">This event is happening today</span>
                        <span className="inline-block w-1 h-1 rounded-full bg-purple-400/60" />
                        <span>Doors open soon</span>
                        <span className="inline-block w-1 h-1 rounded-full bg-purple-400/60" />
                        <span>Secure your spot now</span>
                        <span className="inline-block w-1 h-1 rounded-full bg-purple-400/60" />
                        <span className="text-white/90 font-black tracking-[0.15em]">Last chance to register</span>
                        <span className="inline-block w-1 h-1 rounded-full bg-purple-400/60" />
                ...
                {/* Right: time callout */}
                <div className="flex-shrink-0 flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5">
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-5">
            ...
            {/* Other status messages — unchanged, no emojis */}
            {isSoldOut && (
              ...
              <div className="text-center">
                <div className="bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-300 rounded-2xl p-8 md:p-10 shadow-lg">
                  ...
                  {isEventPassed ? (
                    ...
                    >
                      <CheckCircle size={20} />
                      {isEventToday ? "Register Now — Today Only" : "Get Free Ticket"}
                    </button>
                  )}
